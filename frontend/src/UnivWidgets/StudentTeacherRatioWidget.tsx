import React from 'react';
import { Box, Text, useColorMode } from '@chakra-ui/react';
import { PieChart, Pie, Cell, ResponsiveContainer } from 'recharts';
import PercentileBar from 'src/ReusableComponents/PercentileBar';
import WidgetBox from 'src/containers/WidgetBox';
import { useResponsive } from 'src/containers/useResponsive';
import DataInfoPopover from 'src/ReusableComponents/DataInfoPopover';
import { useGlobalStats } from 'src/helpers/useGlobalStats';

interface StudentTeacherRatioWidgetProps {
  universityName: string;
  studentFacultyRatio: string;
  studentFacultyRatioPercentile: string;
}

const StudentTeacherRatioWidget: React.FC<StudentTeacherRatioWidgetProps> = ({ universityName, studentFacultyRatio, studentFacultyRatioPercentile }) => {
  const { colorMode } = useColorMode();
  const isDark = colorMode === 'dark';

  const hasData = studentFacultyRatio !== 'No Data' && studentFacultyRatio !== '';
  const ratio = parseFloat(studentFacultyRatio) || 0;
  const MAX_RATIO = 30;
  const gauge = Math.min((ratio / MAX_RATIO) * 100, 100);
  const remaining = 100 - gauge;

  const data = [
    { name: 'Ratio', value: gauge },
    { name: 'Remaining', value: remaining }
  ];

  const colorThresholds = [
    { threshold: 10, color: '#59A14F' },    // Green
    { threshold: 15, color: '#EDC949' },    // Yellow
    { threshold: 20, color: '#F28E2B' },    // Orange
    { threshold: Infinity, color: '#E15759' } // Red
  ];

  // Find the first threshold that matches
  const { color } = colorThresholds.find(({ threshold }) => ratio <= threshold) ||
    { color: '#F44336' };

  const { chartRadius } = useResponsive();
  const { inner, outer } = chartRadius;
  const globalStats = useGlobalStats();

  return (
    <WidgetBox
      p={{ base: 2, md: 4 }}
      height="100%"
      position="relative"
    >
      <Text fontSize={{ base: "sm", sm: "md", lg: "lg" }} fontWeight="bold" mb={2}>Student: Faculty</Text>

      <Box height={{ base: "100px", md: "140px" }} position="relative">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie
              data={data}
              cx="50%"
              cy="70%"
              startAngle={180}
              endAngle={0}
              innerRadius={inner}
              outerRadius={outer}
              cornerRadius={5}
              paddingAngle={5}
              dataKey="value"
            >
              <Cell fill={color} stroke={isDark ? 'rgba(20,24,36,0.5)' : 'white'} strokeWidth={1} />
              <Cell fill={isDark ? 'rgba(255,255,255,0.06)' : '#EDF2F7'} stroke={isDark ? 'rgba(20,24,36,0.5)' : 'white'} strokeWidth={1} />
            </Pie>
            <text
              x="50%"
              y="60%"
              textAnchor="middle"
              style={{
                fontSize: 'clamp(16px, 3.5vw, 28px)',
                fontWeight: 'bold',
                fill: color
              }}
            >
              {hasData ? `${ratio}:1` : 'N/A'}
            </text>
            <text x="50%" y="80%" textAnchor="middle" fill={isDark ? "#d1d5db" : "#4b5563"} style={{ fontSize: 'clamp(8px, 1.5vw, 12px)' }}>
              <tspan x="50%" dy="0">Students per</tspan>
              <tspan x="50%" dy="12">Faculty Member</tspan>
            </text>
          </PieChart>
        </ResponsiveContainer>
      </Box>

      <Box mt={2}>
        <PercentileBar
          value={hasData ? parseFloat(studentFacultyRatioPercentile) || 0 : 0}
          name={universityName}
          type="Student-Faculty Ratio"
          colorScheme="inverted"
          globalAvg={globalStats?.undergrad.studentFacultyRatio}
        />
      </Box>

      <DataInfoPopover />
    </WidgetBox>
  );
};

export default StudentTeacherRatioWidget;

...
const NavigationFooter: React.FC<NavigationFooterProps> = ({ mode }) => {
  const { colorMode } = useColorMode();
  const isDark = colorMode === 'dark';
  const { pathname } = useLocation();

  const navItems: Array<{ id: string; path: string; icon: any; label: string }> = [
    { id: 'explore', path: '/', icon: SearchIcon, label: 'UniDB' },
    { id: 'favorites', path: '/favorites', icon: FaHeart, label: 'Favorites' },
    { id: 'chat', path: '/chat', icon: ChatIcon, label: 'UniChat' },
    { id: 'facultydb', path: '/facultydb', icon: StarIcon, label: 'FacultyDB' },
    { id: 'about', path: '/about', icon: InfoIcon, label: 'About' }
  ];
